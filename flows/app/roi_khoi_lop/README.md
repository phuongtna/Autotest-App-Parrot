...
| RKL-06 | TC_06,07,08,09,13,21 | Rời lớp thật + verify toàn bộ hệ quả | Cơ chế cốt lõi **đã chạy PASS thật (2026-09-08)** bằng 1 profile dùng-1-lần trong lớp test: TC_06 (banner "Đã rời khỏi lớp" + Lớp học trống), và **TC_09 xác nhận thật với 1 bài tập THẬT đã giao+đã làm** (không phải lớp rỗng) - sau khi rời, tab "Bài tập" của profile đó hiện đúng "0/0 - Bạn không có bài tập nào đang chờ" ở CẢ 2 bộ lọc ("2 tuần gần nhất" VÀ "1 tháng gần nhất", dùng lại `helpers/homework-select-month-filter.yaml`) - bài `G5-U4-Lesson 3: Listen and match` biến mất hoàn toàn, không chỉ ẩn theo bộ lọc mặc định. Xem TC_14/15/16 bên dưới. **CẬP NHẬT 2026-09-15**: file `RKL-06-leave-class-real-full-verify.yaml` tự nó **đã chạy PASS thật lần đầu tiên trên PRODUCTION** với `PROFILE_WITH_CLASS="Gia Linh"`/`PROFILE_OTHER=Ngoc` (tài khoản `0915775115`) - trước đó file này chưa từng chạy được thật (không phải do thiếu profile `Hang` như ghi chú cũ, mà do 1 bug điều hướng thật: gọi lặp `open-thong-tin-cac-con.yaml` khi không còn đứng trên tab bar khiến tap "Báo cáo" FAILED - đã sửa bằng cách bỏ qua bước mở lại màn hình khi đã đứng sẵn đó, hoặc chèn `launch-keep-session.yaml` trước khi cần quay lại tab bar). File cũng đã bổ sung chụp cả 2 bộ lọc ("2 tuần"/"1 tháng") ở bước BEFORE (không chỉ bộ lọc mặc định) để phục vụ đối chiếu TC_19 sau này. Cùng phiên, đã chạy tiếp **TC_19 (rejoin đúng lớp 3B vừa rời) PASS đầy đủ 3 bước thật trên production** (app chọn lại lớp → GV "Phương" duyệt qua `npm run approve-student-request` → verify) - xác nhận bài CHƯA quá hạn hiện lại bình thường, và bài ĐÃ quá hạn lúc rời lớp (hạn 01/09, 08/09) KHÔNG hiện lại kể cả ở bộ lọc 1 tháng sau khi rejoin. Xem `project_rkl06_production_real_run_2026_09_15` trong memory để biết chi tiết đầy đủ. |
